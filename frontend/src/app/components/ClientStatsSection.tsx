...
const ClientStatsSection: React.FC<ClientStatsSectionProps> = ({ data, selectedStat, onStatClick }) => {
  ...
  const stats = [
    { name: 'total', title: 'Active Clients', value: data.timesheet?.summary?.uniqueClients?.toString() ?? '0' },
  ...
  return (
    <div className="grid grid-cols-6 gap-4">
      <div className="col-span-6">
        <div className={`grid grid-cols-1 ${data.engagements?.clients?.data ? 'lg:grid-cols-6' : 'lg:grid-cols-5'} gap-4`}>
          {data.engagements?.clients?.data && (
            <div className="lg:col-span-1">
              <SectionHeader title="All Time" subtitle="" />
              {renderStat('allClients', 'All Clients', data.engagements.clients.data.length.toString())}
            </div>
          )}
          <div className={data.engagements?.clients?.data ? 'lg:col-span-5' : 'lg:col-span-6'}>
            <SectionHeader title="Active" subtitle="Last Six Weeks" />
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
              {stats.map(stat => renderStat(stat.name, stat.title, stat.value, stat.color, data.timesheet?.summary?.uniqueClients))}
            </div>
          </div>
        </div>
      </div>
