...
def resolve_performance_analysis_pivoted(performance_analysis: PerformanceAnalysis, info):
    return {
        "regular": _compute_performance_analysis_of_regular_cases_pivoted(performance_analysis),
        "pre_contracted": _compute_performance_analysis_of_pre_contracted_cases_pivoted(performance_analysis)
    }

def _compute_performance_analysis_of_regular_cases_pivoted(performance_analysis: PerformanceAnalysis):
...
def _compute_performance_analysis_of_pre_contracted_cases_pivoted(performance_analysis: PerformanceAnalysis):
    # Get unique account manager names across all weeks
    account_managers_names = sorted({
        am.name for week in performance_analysis.weeks 
        for am in week.account_managers
            ...
            for client in am.clients:
                for sponsor in client.sponsors:
                    if sponsor.name not in case_titles_by_sponsor:
                        case_titles_by_sponsor[sponsor.name] = set()
                    case_titles_by_sponsor[sponsor.name].update(
                        case.title for case in sponsor.pre_contracted_cases
                    )
        ...
        for week in performance_analysis.weeks:
            for am in week.account_managers:
                ...
                weekly_totals.append({
                    ...
                    "end": week.end,
                    "period_type": week.period_type,
                    "totals": am.totals.pre_contracted,
                })
                
                # Add client and sponsor weekly data
                for client in am.clients:
                    if not client.totals.pre_contracted:
                        continue
                    ...
                    if matching_client:
                        matching_client["weeks"].append({
                            ...
                            "end": week.end,
                            "period_type": week.period_type,
                            "totals": client.totals.pre_contracted,
                        })
                        
                        # Add sponsor weekly data
                        for sponsor in client.sponsors:
                            if not sponsor.totals.pre_contracted:
                                continue
                            ...
                            if matching_sponsor:
                                matching_sponsor["weeks"].append({
                                    ...
                                    "end": week.end,
                                    "period_type": week.period_type,
                                    "totals": sponsor.totals.pre_contracted,
                                })

                                # Add case weekly data
                                for case in sponsor.pre_contracted_cases:
                                    matching_case = next(
                                        (c for c in matching_sponsor["by_case"] if c["title"] == case.title),
                                        None
                                    )
                                    if matching_case:
                                        matching_case["weeks"].append({
                                            ...
                                            "totals": TotalsPreContracted(
                                                ...
                                                actual_work_hours=case.actual_work_hours,
                                                in_context_actual_work_hours=case.in_context_actual_work_hours,
                                                possible_unpaid_hours=case.possible_unpaid_hours,
                                                possible_idle_hours=case.possible_idle_hours
                                            ),
                                            "actual_work_hours": case.actual_work_hours,
                                            "in_context_actual_work_hours": case.in_context_actual_work_hours,
                                            "possible_unpaid_hours": case.possible_unpaid_hours,
