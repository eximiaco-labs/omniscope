...
def compute_week_review(date_of_interest, filters=None):
    ...
    for i, day in enumerate(week_days):
        result[day] = _compute_timesheet_date_analysis(df, start + timedelta(days=i))

    # Add month summary
    result['month_summary'] = _calculate_month_summary(df, date_of_interest, filters)
    
    summary = _calculate_week_summary(df, date_of_interest, filters)
    ...
    result['allocation_analysis_by_worker'] = _create_six_weeks_allocation_analysis(df, date_of_interest)
    result['allocation_analysis_by_client'] = _create_six_weeks_allocation_analysis(df, date_of_interest, 'ClientName')

    return result

def _calculate_week_summary(six_weeks_df: pd.DataFrame, date_of_interest: datetime, filters=None):
...
def _calculate_month_summary(six_weeks_df, date_of_interest, filters=None):
...
def _create_six_weeks_allocation_analysis(
        df: pd.DataFrame,
...
def _compute_timesheet_date_analysis(df, date_of_interest: datetime, number_of_weeks: int = 6):
    ...
    daily_summary = ds.groupby(['Date', 'Kind'])['TimeInHs'].sum().unstack().fillna(0)

    all_dates_df = pd.DataFrame(0, index=dates_of_interest, columns=kinds_present)
    all_dates_df.index.name = 'Date'

    daily_summary = pd.concat([daily_summary, all_dates_df]).groupby('Date').sum().fillna(0)
    ...
    total_hours = current_day_data['TimeInHs'].sum()
    ...
    best_day = ds_grouped.idxmax()
    best_day_hours = ds_grouped.max()

    worst_day = ds_grouped.idxmin()
    worst_day_hours = ds_grouped.min()

    average_hours = ds_grouped.mean()
